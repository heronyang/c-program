 ...
 *
 * NOTE: error handlers are modified for proxy.c which ignore the errors
 * and never triggers exit
 *
 * Updated 8/2014 droh: 
...
{
    // Never let the process exit unexpected
    fprintf(stderr, "%s: %s\n", msg, strerror(errno));
}
/* $end unixerror */
...
{
    // Never let main thread exit unexpected
...
ssize_t Rio_readn(int fd, void *ptr, size_t nbytes) 
{
    ...
    if ((n = rio_readn(fd, ptr, nbytes)) < 0) {
        // Don't let process exit on rio read error
...
int Open_clientfd(char *hostname, char *port) 
{
    ...
    // Don't let the process exit on this kind of error
